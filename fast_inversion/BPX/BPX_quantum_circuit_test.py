...
from qiskit.circuit.library import StatePreparation, CXGate, XGate, ZGate, QFT, HGate, RYGate, IntegerComparator, DraperQFTAdder
...
# Assumed that this can be block-encoded because of the consistent structure of it
...
def get_Lu(n, full=False):
    ...
    N_col = int(2**(n+1) - n - 2)
    if full:
        Lu = np.zeros((2**(n+1),2**(n+1)))
    else:
        Lu = np.zeros((N_row,N_col))
    section = 0
    for col_i in range(N_col):
        if col_i > 2**(section+2)-section-4:
            section+=1
        row_i = int((col_i-(2**(section+1)-section-2)+1) * (N/(2**(section+1))) - 1) # (i-(2**section-1)+1) = column number of section, (N/(2**(section+1))) = # of rows gap between each column in section
        Lu[row_i,col_i] = 1
    if full:
        for i in range(N_col,2**(n+1)):
            Lu[i,i] = 1
    return Lu
